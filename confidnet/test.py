...
def main():
    ...
    parser.add_argument("--mean_last", type=int, default=None, help="Average results over last epochs")
    ...
    if args.mean_last is not None:
        ...
        learner.metrics = metrics_to_averaged
        
        # Initialize and load model
        list_ckpts = sorted(os.listdir(config_args["training"]["output_folder"] / "ckpts"), key= lambda x: int(x.split('.')[0].split('_')[-1]))[-args.mean_last:]
        ...
        for ckpt_name in list_ckpts:
            ...
            LOGGER.info(f"Evaluating model {ckpt_name}")

            # trust score not supported when averaging last models
            results = learner.evaluate(
                learner.test_loader,
                learner.prod_test_len,
                split="test",
            ...
            for st in scores_test:
                averaged_results[st].append(scores_test[st]['value'])
            ...
            if args.val_temp_scaling:
                ...
                scores_test_val_ts = results_val_ts[1]
                for st in scores_test_val_ts:
                    averaged_results_val_ts[st].append(scores_test_val_ts[st]['value'])
        ...
        for st in averaged_results:
            print(st)
            print(f"mean: {np.mean(averaged_results[st])*100:.2f}, std: {np.std(averaged_results[st])*100:.4f}")
            print("----------------------------------------------------------------")
        ...
        if args.val_temp_scaling:
            ...
            for st in averaged_results_val_ts:
                print(st)
                print(f"mean: {np.mean(averaged_results_val_ts[st])*100:.2f}, std: {np.std(averaged_results_val_ts[st])*100:.4f}")
                print("----------------------------------------------------------------")
        ...
        if args.opt_temp_scaling:
            ...
            for st in averaged_results_opt_ts:
                print(st)
                print(f"mean: {np.mean(averaged_results_opt_ts[st])*100:.2f}, std: {np.std(averaged_results_opt_ts[st])*100:.4f}")
                print("----------------------------------------------------------------")

            # get_confidences not supported when averaging last models
            # if args.get_confidences:
            #     conf = results[2]
